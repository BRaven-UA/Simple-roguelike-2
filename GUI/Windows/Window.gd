...
export var tooltip_data: Resource	#actually it's TooltipData resource, which cannot be exported yet (as custom class)
...
func _ready():
	...
	if not tooltip_data:
		tooltip_data = Global.tooltip.data_template
...
func _content_input(event: InputEvent):
	...
	if event is InputEventMouseMotion and Input.get_mouse_button_mask() == 0:
		if event.relative:	# workaround for Windows bug when InputEventMouseMotion generated on mouse buttons release (https://github.com/godotengine/godot/issues/20357)
